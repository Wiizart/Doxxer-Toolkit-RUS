...
▀▀▀▀▀•  ▀█▄▀▪•▀▀ ▀▀•▀▀ ▀▀ ▀▀▀ .▀  ▀     ▀▀▀  ▀█▄▀▪ ▀█▄▀▪.▀▀▀ ·▀  ▀▀▀▀ ▀▀▀ 
               ...
               Development by:  Euronymou5
               
               Translation by:  Wizart
                    _______________
...
def sher():
  os.system("clear")
  print(logo)
  print('\n[1] Поиск с помощью Doxxer Toolkit')
  print('[2] Поиск с помощью Sherlock (Program)')
  print('[3] Поиск с помощью Nexfil (Program)')
  print('[4] Базовый поиск с помощью Socialscan')
  ...
  if var == 1:
     ...
     user = input('\n[~] Введите пользователя для поиска: ')
...
def iplog():
  ...
  opc = int(input('>> '))
  if opc == 1:
    print('\n[1] Открыть ссылку для Linux')
    print('\n[2] Открыть ссылку для Termux')
    print('\n[00] Назад в меню')
    print('\n[99] Выход')
    Skd = int(input('>> '))
    if Skd == 1:
...
def fakerr():
  ...
  [2] Создать поддельный номер телефона
  
  [3] Создать профиль фейкового человека
  
  [4] Генерация пользовательских агентов

  [5] Создать поддельную кредитную карту
  ...
  if fakk == 1:
    ...
    fake = Faker()
    Faker.seed(0)
    print('[~] Сколько раз вы хотите сгенерировать поддельный номер?')
    num = int(input('[~] Введите номер: '))
    print('[~] Создание поддельного номера телефона...')
    time.sleep(1)
    for _ in range(num):
    ...
    fake = Faker()
    Faker.seed(0)
    print('[~] Сколько раз вы хотите создать поддельный профиль?')
    num = int(input('[~] Введите номер: '))
    print('[~] Создание фейкового профиля...')
    time.sleep(1)
    for _ in range(num):
      print(fake.profile())
    fakerr()
    ...
    fake = Faker()
    Faker.seed(0)
    print('[~] Сколько раз вы хотите создать пользовательского агента?')
    num = int(input('[~] Введите номер: '))
    print('[~] Генерация пользовательского агента...')
    time.sleep(1)
    for _ in range(num):
    ...
    fake = Faker()
    Faker.seed(0)
    print('[~] Сколько раз вы хотите создать кредитную карту?')
    num = int(input('[~] Введите номер: '))
    print('[~] Создание поддельной кредитной карты...')
    time.sleep(1)
    for _ in range(num):
...
def phishing():
  os.system("clear")
  print(logo)
  print('[1] Использовать Doxxer-Toolkit фишинг')
  print('[2] Zphisher (Program)')
  print('[3] 0ni-Phish (Program) (Только испанский)')
  print('[00] Назад в меню')
  print('[99] Выход')
  var = int(input('\n>> '))
  if var == 1:
     print('''
     [~] Выберите язык на странице для использования:
...
def menu():
    ...
    [~] Добро пожаловать в Doxxer Toolkit!
                      
    [1] IPloggers      
    [2] Геолокация IP           
    [3] Получить информацию по номеру телефона
    [4] Фишинг       
    [5] СМС                        
    [6] Анонимные электронные письма   
    [7] Поиск пользователей 
    [8] Сгенерировать фейковую информацию
    [9] OSINT страницы (BETA)
    [10] Сократить ссылку
    [11] Сгенерировать QR код
    [12] OSINT email (BETA)
    [99] Выход
    ''')
    elejir = int(input('\n>> '))
